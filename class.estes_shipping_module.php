...
class estes_shipping_module {
	...
	function getQuote() {
		...
		// validation
		if (empty($country) || empty($zipcode) || empty($state) || empty($weight))
			return array();
		...
		if ($cache != null && !empty($cache)) {
			if ($cache['country'] === $country && $cache['zipcode'] === $zipcode && $cache['state'] === $state && $cache['weight'] === $weight) {
				$rates = $cache['rates'];
		...
		$rates = $this->_makeRequest($country, $zipcode, $state, $weight);
		
		// build cache values
		$values = array('country' => $country, 'zipcode' => $zipcode, 'state' => $state, 'weight' => $weight, 'rates' => $rates);
    ...
    protected function _makeRequest($country, $zip, $state, $weight) {
        ...
        try {
			...
			$request = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:rat="http://ws.estesexpress.com/ratequote" xmlns:rat1="http://ws.estesexpress.com/schema/2012/12/ratequote"><soapenv:Header><rat:auth><rat:user>' . $options['username'] . '</rat:user><rat:password>' . $options['password'] . '</rat:password></rat:auth></soapenv:Header><soapenv:Body><rat1:rateRequest><rat1:requestID>' . $requestID . '</rat1:requestID><rat1:account>' . $options['account'] . '</rat1:account><rat1:originPoint><rat1:countryCode>' . $options['countryCode'] . '</rat1:countryCode><rat1:postalCode>' . $options['zip'] . '</rat1:postalCode><rat1:city>' . $options['city'] . '</rat1:city><rat1:stateProvince>' . $options['state'] . '</rat1:stateProvince></rat1:originPoint><rat1:destinationPoint><rat1:countryCode>' . $country . '</rat1:countryCode><rat1:postalCode>' . $zip . '</rat1:postalCode><rat1:stateProvince>' . $state . '</rat1:stateProvince></rat1:destinationPoint><rat1:payor>S</rat1:payor><rat1:terms>PPD</rat1:terms><rat1:stackable>N</rat1:stackable><rat1:baseCommodities><rat1:commodity><rat1:class>' . $class . '</rat1:class><rat1:weight>' . $weight . '</rat1:weight></rat1:commodity></rat1:baseCommodities></rat1:rateRequest></soapenv:Body></soapenv:Envelope>';
